...
$where = " WHERE 1 ";
	...
	if($searchField=="plan"){
		$where .= " AND plan = '".mysql_real_escape_string($searchString)."' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from premium_master  ".$where;
//echo $Query;
...
$Query = "select  *  from premium_master ".$where.$OrderBY.$Limit;
echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
  ...
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="plan" <? if($searchField=='plan') echo 'selected';?>>Premium </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=plan&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Tenure (Months) </TD>
...
<!-- <TD align=center width="12%" class="heading_text">Delete</TD> -->
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['plan']?></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM premium_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
			...
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				...
				<td width="75%" align="left" valign="top">
					...
					<?php
						$selPlan = mysql_query("SELECT * FROM insurance_plan");	
				...
				<?php 
					if($numPlan > 0)
					{
						...
						{							
				?>
					<option value="<?php echo $getPlan['id']; ?>" <?php echo ($getPlan['id'] == $rec[0]['plan'] ? 'selected' : ''); ?>><?php echo $getPlan['plan_name']; ?></option>
				<?php
						}
					}
				?>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from premium_master WHERE plan = '".$plan."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO premium_master SET ";
		$Query .= " plan 							= '".$plan."',";
	...
	else
	{
		...
		$Query  = "select * from premium_master WHERE plan = '".$plan."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Data already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
				
					
		$Query  = " UPDATE premium_master SET ";
		$Query .= " plan							= '".$plan."',";
		$Query .= " bonus_percentage							= '".$bonus_percentage."'";
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from premium_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE premium_master SET status = 0, deactivation_date='".date('Y-m-d')."' WHERE id = ".$row_id."";
	#mysql_query("UPDATE premium_master SET deactivation_date='".date('Y-m-d')."'");
	...
	$Query  = "UPDATE premium_master SET status = 1, deactivation_date='0000-00-00' WHERE id = ".$row_id."";
	}
	...
	$_SESSION[SUCCESS_MSG] = "Record Changed successfully...";
