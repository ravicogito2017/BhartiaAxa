...
function show_list(){
	...
	$sortField 		= loadVariable('sf','place');
...
$where = " WHERE 1 ";
	...
	if($searchField=="place"){
		$where .= " AND place like '%".mysql_real_escape_string($searchString)."%' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from place_master  ".$where;
...
$Query = "select  *  from place_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Place Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="place" <? if($searchField=='place') echo 'selected';?>>Place </option></td>
      <TD width="85%">
...
<!-- <TD align=center width="12%" class="heading_text">Delete</TD> -->
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['place']?></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM place_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
			...
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				...
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="place" id="place" size="40" <? if(isset($rec[0]['place']) && $rec[0]['place']!="") {?> value="<?=stripslashes($rec[0]['place'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from place_master WHERE place = '".$place."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Place already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
				
					
		$Query  = " INSERT INTO place_master SET ";
		$Query .= " place 							= '".$place."'";
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "Place Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$Query  = "select * from place_master WHERE place = '".$place."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$_SESSION[SUCCESS_MSG] = "Place updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
