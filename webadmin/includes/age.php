...
$where = " WHERE 1 ";
	...
	if($searchField=="age"){
		$where .= " AND age like '%".mysql_real_escape_string($searchString)."%' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from age_master  ".$where;
...
$Query = "select  *  from age_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Age Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="age" <? if($searchField=='age') echo 'selected';?>>Age </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=age&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=age&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Age </TD>
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['age']?></TD>
	  <!-- <TD align=center>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM age_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
			...
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				...
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="age" id="age" size="40" <? if(isset($rec[0]['age']) && $rec[0]['age']!="") {?> value="<?=stripslashes($rec[0]['age'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from age_master WHERE age = '".$age."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO age_master SET ";
		$Query .= " age 							= '".$age."'";
	...
	else
	{
		...
		$Query  = "select * from age_master WHERE age = '".$age."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from age_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE age_master SET status = 0 WHERE id = ".$row_id."";
	...
	$Query  = "UPDATE age_master SET status = 1 WHERE id = ".$row_id."";
	}
