...
if(isset($_GET['status']) && $_GET['status'] == 'del' && $_GET['did'] != "") {
    ...
    $update = "update pis_master_ge_renewal set is_deleted = '1' where id = ".$id;    
    $objDB->setQuery($update);
    $objDB->update();        
    
    $select = "select pis_mode from pis_master_ge_renewal where id = ".$id;    
    ...
    $update1 = "update installment_master_ge_renewal set  ".$updcol."  = '0' where  $updcol = ".$id;     
    $objDB->setQuery($update1);
    $objDB->update(); 
    
    header("Location:".URL."webadmin/index.php?p=pis_branch_renewal_ge");   
  ...
  <tbody>
    <tr> 
      ...
      <td colspan="20" align="right">
        <a title=" Export to Excel " href="<?=URL?>pis_excel_renewal_ge.php" ><img src="images/excel_icon.gif" border="0" title="Download" /></a>
      </td>
    </tr>
		<?php
			}	
...
?>
	...
	<tr> 			
			...
			<?php if($_SESSION[ROLE_ID] == '1' || $_SESSION[ROLE_ID] == '4'){ ?>			
			<td width="8%" align="center" valign="top" ><a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_pis_admin_renewal_ge.php?id=<?php echo base64_encode($mysql_id); ?>')">View</a></td>
			<?php }
			if($_SESSION[ROLE_ID] == '1'){ ?>
			<td width="8%" align="center" valign="top" >
			
			<a href="<?php echo URL; ?>webadmin/index.php?p=pis_branch_renewal_ge&status=del&did=<?php echo base64_encode($mysql_id); ?>" onclick="return confirm('Do you want to delete this item?');">Cancel</a>
			</td>
			<?php } ?>
    </tr>
<?php		
