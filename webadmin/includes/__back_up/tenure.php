...
//=======================================================
$Query = "select count(id) as CNT from tenure_master  ".$where;
...
$Query = "select  *  from tenure_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Tenure (Years) </TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM tenure_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Tenure (Years) <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="tenure" id="tenure" size="40" <? if(isset($rec[0]['tenure']) && $rec[0]['tenure']!="") {?> value="<?=stripslashes($rec[0]['tenure'])?>" <? }?> onKeyPress="return keyRestrict(event, '0123456789')" />
				</td>
			  </tr>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from tenure_master WHERE tenure = '".$tenure."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Tenure already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
				
					
		$Query  = " INSERT INTO tenure_master SET ";
		$Query .= " tenure 							= '".$tenure."',";
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "Tenure Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$Query  = "select * from tenure_master WHERE tenure = '".$tenure."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " UPDATE tenure_master SET ";
		$Query .= " tenure							= '".$tenure."',";
		$Query .= " bonus_percentage							= '".$bonus_percentage."'";
		...
		$_SESSION[SUCCESS_MSG] = "Tenure updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
