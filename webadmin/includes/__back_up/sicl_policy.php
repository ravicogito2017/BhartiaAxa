...
//=======================================================
$Query = "select count(id) as CNT from sicl_app_master  ".$where;
...
$Query = "select  *  from sicl_app_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
  ...
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR> 
	  ...
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=sicl_policy&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM sicl_app_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from sicl_app_master WHERE application_no = '".$application_no."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO sicl_app_master SET ";
	...
	else
	{
		...
		$Query  = "select * from sicl_app_master WHERE application_no = '".$application_no."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " UPDATE sicl_app_master SET ";
		$Query .= " application_no							= '".$application_no."',";
