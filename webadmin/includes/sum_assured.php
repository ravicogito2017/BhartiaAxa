...
function show_list(){
	...
	$sortField 		= loadVariable('sf','sum_assured');
...
$where = " WHERE 1 ";
	...
	if($searchField=="sum_assured"){
		$where .= " AND sum_assured like '%".mysql_real_escape_string($searchString)."%' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from sum_assured_master  ".$where;
...
$Query = "select  *  from sum_assured_master ".$where.$OrderBY.$Limit;
//echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
//print_r($rs);
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Sum Assured Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="sum_assured" <? if($searchField=='sum_assured') echo 'selected';?>>Sum Assured </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=sum_assured&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=sum_assured&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Sum Assured </TD>
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['sum_assured']?></TD>
	  <!-- <TD align=center>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM sum_assured_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Sum Assured <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="sum_assured" id="sum_assured" size="40" <? if(isset($rec[0]['sum_assured']) && $rec[0]['sum_assured']!="") {?> value="<?=stripslashes($rec[0]['sum_assured'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
...
function  update_record($row_id = '')
{
	...
	$sum_assured 					= loadVariable('sum_assured','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from sum_assured_master WHERE sum_assured = '".$sum_assured."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO sum_assured_master SET ";
		$Query .= " sum_assured 							= '".$sum_assured."'";
	...
	else
	{
		...
		$Query  = "select * from sum_assured_master WHERE sum_assured = '".$sum_assured."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " UPDATE sum_assured_master SET ";
		$Query .= " sum_assured							= '".$sum_assured."'";
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from sum_assured_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE sum_assured_master SET status = 0 WHERE id = ".$row_id."";
	...
	$Query  = "UPDATE sum_assured_master SET status = 1 WHERE id = ".$row_id."";
	}
