...
if(isset($branch_despatched) && count($branch_despatched) > 0)
{
	...
	$sql= "UPDATE direct_renewal_premium_gtfs SET branch_despatched=1, branch_despatch_date='".$today."' WHERE id IN (".$branch_despatchedStr.")";
	#echo $sql;
	//exit;
...
$pid 			= loadVariable('pid',0);
	...
	{
		$lastNameString = find_id_through_last_name($_SESSION['applicant_name']);
		#echo $lastNameString;
		if($lastNameString != '')
		{
			$where.= ' AND applicant_name like  IN %'.$_SESSION['applicant_name'].'%';
		}
	}
...
<TABLE class="table_border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <tbody>
    ...
    <tr class="TDHEAD"> 
      <td colspan="28">Transaction Listing</td>
    </tr>
		<tr>
	...
	?>
      <td width="8%" align="center" valign="top" class="tbllogin">Hub Received</td>
			<td width="8%" align="center" valign="top" class="tbllogin">Hub Despatched</td>
...
?>
	<tr> 
			<td width="8%" align="center" valign="top" ><input type="checkbox" name="branch_despatched[]" value="<?php echo $mysql_id; ?>"<?php if($rs[$i]['branch_despatched'] == '1'){ echo 'disabled="disabled"';}?> <?php echo ($rs[$i]['branch_despatched'] == '1' ? 'checked' : '')?>
		...
		if($rs[$i]['hub_despatched'] == 1) {$status="Despatched to Hub"; }
