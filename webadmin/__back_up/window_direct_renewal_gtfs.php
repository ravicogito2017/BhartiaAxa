...
function find_receipt_status($receipt_id)
{
	...
	//echo $invoice_id;
	$selTransaction = mysql_query("SELECT receipt_generated FROM direct_renewal_premium_gtfs WHERE id='".$receipt_id."'");
				...
				<table>
					<tr>
					  ...
					  <td width="552" align="center">
						...
						<p style="font-size: 16px;" align="center"><?php 
	...
	<br/><br/>
	 ...
	 if(isset($getTransaction['applicant_name'])) {
		...
		<?php if(($getTransaction['sp_payment_mode'] == 'CHEQUE') || ($getTransaction['sp_payment_mode'] == 'DD')){?>Cheque/Demand Draft No <?php echo $getTransaction['sp_dd_no']; ?> dated <?php echo date('d-m-Y',strtotime($getTransaction['sp_dd_date'])); ?> drown on <?php echo $getTransaction['sp_dd_bank']; ?><?php }?>
			towards RP(<?php if($getTransaction['is_duplicate']=='1'){echo "Duplicate Entry Premium";}else{echo "Direct Renewal Premium";}?>) in relation to above Proposal for  & on behalf of Life Insurance Corporation of India under<strong> <?php echo $data['plan_name'];?> </strong> and term<strong> <?php echo $getTransaction['term']; ?></strong> Years.</div>
	<br/>
<br/><br/>
...
<?php
	mysql_query("UPDATE direct_renewal_premium_gtfs SET receipt_generated=1 WHERE id='".base64_decode($_GET['id'])."'");
