...
$redemption_doc_verified_by_branch = 0 ;
$paying_for_extra_grams = 0;
...
if(isset($_GET['id']) && !empty($_GET['id']))
{
	...
	if(isset($_POST['redemption_reason']) && $_POST['redemption_reason'] != '')
	{
		...
		#exit;
		extract($_POST);
		$redemption_doc_verified_by_branch = isset($chkVerified) ? '1' : '0'; 
		$paying_for_extra_grams = isset($chkAgreed) ? '1' : '0'; 
		
				$folio_query = "UPDATE customer_folio_no SET applied_for_redemption = 1, redemption_applied_by = '".$redemption_applied_by."', redemption_doc_verified_by_branch = '".$redemption_doc_verified_by_branch."', redemption_reason = '".$redemption_reason."', paying_for_extra_grams='".$paying_for_extra_grams."',redemption_date = '".date('Y-m-d')."', redemp_emp_code='".$redemp_emp_code."', redemp_emp_name='".$redemp_emp_name."' WHERE id='".$folio_id."'";
				$update_folio = mysql_query($folio_query);

				//echo $folio_query.'<br />';

				if(intval($paying_for_extra_grams) == 0)
				{
					$extra_price = 0.00;
					$extra_tax = 0.00;
					$extra_gram = 0.0000;					
				...
				$transaction_id = $product_name.'/'.$branch_code.'/'.str_pad(($branch_transaction + 1),6,'0',STR_PAD_LEFT).'/'.date('m/Y');
				...
				$insert_installment = "INSERT INTO installment_master SET 
										...
										customer_id = '".mysql_real_escape_string($getFolioRecord['customer_id'])."',
										...
										payment_mode = 'CASH',
										amount = '".$extra_price."',
										transaction_charges = '".$extra_tax."',
										gold_gram = '".$extra_gram."',
...
##############################
			...
			$redemp_emp_code = $getFolioRecord['redemp_emp_code'];
			$redemp_emp_name = $getFolioRecord['redemp_emp_name'];
			$redemption_doc_verified_by_branch = $getFolioRecord['redemption_doc_verified_by_branch'];
			$redemption_applied_by = $getFolioRecord['redemption_applied_by'];
			$redemption_doc_verified_by_branch = $getFolioRecord['redemption_doc_verified_by_branch'];
			$paying_for_extra_grams = $getFolioRecord['paying_for_extra_grams'];
...
<!--
	...
	function dochk()
	{	
		if(document.addForm.redemp_emp_name.value.search(/\S/) == -1)
		{
			alert("Please Select Emp Name for Branch");
			document.addForm.redemp_emp_name.focus();
			return false;
		}	
		if(document.addForm.redemp_emp_code.value.search(/\S/) == -1)
		{
			alert("Please Select Emp Code for Branch");
			document.addForm.redemp_emp_code.focus();
			return false;
		}	
		if(document.addForm.redemption_reason.value.search(/\S/) == -1)
		{
			alert("Please Select Redemption Reason");
			document.addForm.redemption_reason.focus();
			return false;
		}	
		if(document.addForm.redemption_applied_by.value.search(/\S/) == -1)
		{
			alert("Please Select the Applicant");
			document.addForm.redemption_applied_by.focus();
			return false;
		}	
		if(document.addForm.chkVerified.checked == false)
		{
			alert("Branch user must verify all the documents");
			document.addForm.chkVerified.focus();
			return false;
		}	
	}
//-->
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr> 
      ...
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><?php echo $fathers_name; ?></td>
    </tr>

		<tr> 
      <td class="tbllogin" valign="top" align="right">Mother's Maiden Name</td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><?php echo $mothers_maiden_name; ?></td>
    </tr>
	...
	<tr> 
      <td class="tbllogin" valign="top" align="right">Guardian Relationship</td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><?php echo $appointee_relationship_type; ?></td>
    </tr>		
	
	<tr> 
      <td class="tbllogin" valign="top" align="right">Emp Name for Branch<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="text" class="inplogin" name="redemp_emp_name" id="redemp_emp_name" value="<?php echo $redemp_emp_name; ?>" onKeyUp="this.value = this.value.toUpperCase();" ></td>
    </tr>
	
	<tr> 
      <td class="tbllogin" valign="top" align="right">Emp Code for Branch<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="text" class="inplogin" name="redemp_emp_code" id="redemp_emp_code" value="<?php echo $redemp_emp_code; ?>" onKeyPress="return keyRestrict(event, '0123456789')" /></td>
    </tr>

		<tr> 
      <td class="tbllogin" valign="top" align="right">Reason For Redemption<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left">
			
			<select id="redemption_reason" name="redemption_reason" class="inplogin">
				<option value="">SELECT</option>
				<option value="DEATH" <?php echo ($redemption_reason == 'DEATH' ? 'selected' : '');?>>DEATH</option>
				<option value="DISCONTINUE" <?php echo ($redemption_reason == 'DISCONTINUE' ? 'selected' : '');?>>DISCONTINUE</option>
				<option value="TERM CLOSED" <?php echo ($redemption_reason == 'TERM CLOSED' ? 'selected' : '');?>>TERM CLOSED</option>
			</select>
			</td>
    </tr>

		<tr> 
      <td class="tbllogin" valign="top" align="right">Applied By<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left">
			
			<select id="redemption_applied_by" name="redemption_applied_by" class="inplogin">
				<option value="">SELECT</option>
				<option value="CUSTOMER" <?php echo ($redemption_applied_by == 'CUSTOMER' ? 'selected' : '');?>>CUSTOMER</option>
				<option value="NOMINEE" <?php echo ($redemption_applied_by == 'NOMINEE' ? 'selected' : '');?>>NOMINEE</option>
				<option value="GUARDIAN" <?php echo ($redemption_applied_by == 'GUARDIAN' ? 'selected' : '');?>>GUARDIAN</option>
			</select>
			</td>
    </tr>

		<tr> 
      <td class="tbllogin" valign="top" align="right"><input type="checkbox" name="chkVerified" value="1" <?php echo ($redemption_doc_verified_by_branch == '1' ? 'checked' : '')?>></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><font color="#ff0000">*</font>Scanned images and other documents verified by the branch user.</td>
    </tr>	

		<tr> 
      <td class="tbllogin" valign="top" align="right"><input type="checkbox" name="chkAgreed" value="1" <?php echo ($paying_for_extra_grams == '1' ? 'checked' : '')?>></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left">Customer is agreed to pay the cost and the service tax for the additional grams.</td>
    </tr>	
