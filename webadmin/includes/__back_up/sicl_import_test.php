...
function find_application_id($application_no) // This function will determine whether this is coming from GAP
	{
		$objDB = new DB();
		$gap_status = 0;
		$selGap_status = mysql_query("SELECT id FROM sicl_app_master_test WHERE 	application_no='".$application_no."'");
	...
	if(isset($_FILES['zip']) && $_FILES['zip']['tmp_name'] != '')
	{
		...
		for ($x = 2; $x <= count($data->sheets[0]["cells"]); $x++) {
			...
			$firstInsert = "INSERT INTO sicl_app_master_test SET
			...
			{
				$firstUpdate = "update sicl_app_master_test SET 
