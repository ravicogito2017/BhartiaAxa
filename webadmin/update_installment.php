...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_folio = "UPDATE  customer_folio_no SET total_premium_given = '".$charge."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT total_premium_given FROM customer_folio_no WHERE id='".$app_id."'";
	$objDB->setQuery($selTransaction);
	$rs = $objDB->select();

	//print_r($rs);
	$value = $rs[0]['total_premium_given'];
	//exit;
...
<html>
 <head>
  <title> Edit Total Premium Given </title>
...
<!--
	...
	{	
		...
		{
			alert("Please Enter Total No. of Premium.");
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Total Premium Given</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Total No. of Premium</td>
      <td width="5%" align="center" valign="top" class="tbllogin">:</td>
      <td width="68%" align="left" valign="top"><input type="textbox" id="charge" name="charge" value="<?php echo $value; ?>" onKeyPress = "return keyRestrict(event, '0123456789')" maxlength="3" /></td>
    </tr>
