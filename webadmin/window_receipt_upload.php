...
if(isset($_GET['id']) && !empty($_GET['id']))
{
	...
	if(isset($_POST['signature_hidden']))
	{
		...
		extract($_POST);
				...
				$folio_query = "UPDATE customer_folio_no SET scanned_delivery_receipt = '".$form_image_name."', identity_card_copy = '".$signature_image_name."' WHERE id='".$folio_id."'";
...
$getFolioRecord = findFolioData($folio_id); // THIS ARRAY CONTAIN ALL RECORDS FOR A PARICULAR ID
	...
	$application_form_image = $getFolioRecord['scanned_delivery_receipt'];
	$signature_image = $getFolioRecord['identity_card_copy'];
...
<html>
 <head>
  <title> Upload Delivery Receipt Scanned Image </title>
...
<!--
	...
	{	
		...
		{
			alert("Please Select The Scanned Image of the Delivery Receipt");
		...
		if(document.addForm.form_image.value.search(/\S/) != -1)
		{
			...
			var2 = (document.addForm.app_no.value + "_del.pdf").toUpperCase();
			//alert(var1);
			//alert(var2);
			if(var1 != var2)
			{
				alert("Name of the Scanned Image for the Delivery Receipt will be " + var2);
		...
		{
			alert("Please Select The Scanned Image of the Receiver ID");
			document.addForm.signature.focus();
			return false;
		}
		if(document.addForm.signature.value.search(/\S/) != -1)
		{
			...
			var2 = (document.addForm.app_no.value + "_id.pdf").toUpperCase();
			//alert(var1);
			//alert(var2);
			if(var1 != var2)
			{
				alert("Name of the Scanned Image for the Receiver ID will be " + var2);
				document.addForm.signature.focus();
				return false;
			}
		}
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()" enctype="multipart/form-data">	 
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Upload Scanned Image of the Delivery Receipt and Receiver's Identity Card</td>
    </tr>
    ...
    <tr> 
      ...
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><?php echo $first_name.' '.$middle_name.' '.$last_name ; ?></td>
    </tr>

		<tr> 
      <td class="tbllogin" valign="top" align="right">Delivery Receipt Image (.PDF)<br /><font color="#ff0000">(Format : Appliaction Number_DEL.PDF)</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="file" name="form_image" id="form_image" /></td>
    </tr>
		<?php if(!empty($application_form_image)){ ?>
		<tr>       
      <td valign="top" align="center" colspan="3"><!-- <img src="scanned_images/<?php echo $application_form_image; ?>" alt="No image uploaded" /> --><a href="<?php echo URL; ?>webadmin/scanned_images/<?php echo $application_form_image; ?>" >View Delivey Receipt Image</a></td>
    </tr>
		<?php } ?>
		
		<tr> 
      <td class="tbllogin" valign="top" align="right">Receiver's ID Image (.PDF)<br /><font color="#ff0000">(Format : Appliaction Number._ID.PDF)</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="file" name="signature" id="signature" /></td>
		...
		<tr>       
      <td valign="top" align="center" colspan="3"><!-- <img src="scanned_images/<?php echo $signature_image; ?>" alt="No image uploaded" /> --><a href="<?php echo URL; ?>webadmin/scanned_images/<?php echo $signature_image; ?>" >View Receiver's ID Image</a></td>
    </tr>
		<?php } ?>
    <?php
