...
if(isset($branch_scan) && count($branch_scan) > 0)
{
	...
	$receivedUpdate = mysql_query("UPDATE installment_master_branch SET branch_scan=1,  branch_scan_date='".$today."' WHERE id IN (".$receiveStr.") ");
}
  ...
  <tbody>
    <tr> 
      ...
      <td colspan="25" align="right">
        <a title=" Export to Excel " href="<?=URL?>transaction_excel_fresh_sz.php" ><img src="images/excel_icon.gif" border="0" title="Download" /></a>
      </td>
    </tr>
	<tr> 
...
?>
	<tr> 
			...
			<?php if($_SESSION[ROLE_ID] == '1' || $_SESSION[ROLE_ID] == '4'){ ?>
			<td width="8%" align="center" valign="top" > <a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_edit_short.php?id=<?php echo base64_encode($mysql_id); ?>')">Edit</td>
			<?php }
			if($_SESSION[ROLE_ID] == '1'){ ?>
			<td width="8%" align="center" valign="top" >
			
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/short_premium_cancel.php?id=<?php echo base64_encode($mysql_id); ?>')">Cancel</a>
			</td>
			<?php } ?>
