...
//=======================================================
$Query = "select count(id) as CNT from income_proof  ".$where;
//echo $Query;
...
$Query = "select  *  from income_proof ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Income Proof Management Panel</TD>
    </TR>
    <TR> 
	  ...
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=income_proof&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=income_proof&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM income_proof WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from income_proof WHERE document_name = '".$document_name."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO income_proof SET ";
	...
	else
	{
		...
		$Query  = "select * from income_proof WHERE document_name = '".$document_name."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from income_proof WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE income_proof SET status = 0 WHERE id = ".$row_id."";
	...
	$Query  = "UPDATE income_proof SET status = 1 WHERE id = ".$row_id."";
	}
