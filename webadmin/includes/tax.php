...
$Query = "SELECT t.id as id, t.tax_percentage AS tax_percentage, p.place as place_id FROM tax_master AS t, place_master AS p WHERE t.place_id = p.id ORDER BY p.place ASC".$Limit;
#echo $Query ;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from tax_master WHERE place_id = '".$place_id."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
	...
	else
	{
		...
		$Query  = "select * from tax_master WHERE place_id = '".$place_id."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
