...
function show_list(){
	...
	$sortField 		= loadVariable('sf','commodity');
...
$where = " WHERE 1 ";
	...
	if($searchField=="commodity"){
		$where .= " AND commodity like '%".mysql_real_escape_string($searchString)."%' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from commodity_master  ".$where;
...
$Query = "select  *  from commodity_master ".$where.$OrderBY.$Limit;
//echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Commodity Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="commodity" <? if($searchField=='commodity') echo 'selected';?>>Commodity </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=commodity&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<!-- <TD align=center width="12%" class="heading_text">Delete</TD> -->
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['commodity']?></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM commodity_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Commodity Name <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="commodity" id="commodity" size="40" <? if(isset($rec[0]['commodity']) && $rec[0]['commodity']!="") {?> value="<?=stripslashes($rec[0]['commodity'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
...
function  update_record($row_id = '')
{
	...
	$commodity 					= loadVariable('commodity','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from commodity_master WHERE commodity = '".$commodity."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Commodity already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
				
					
		$Query  = " INSERT INTO commodity_master SET ";
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "Commodity Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$Query  = "select * from commodity_master WHERE commodity = '".$commodity."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " UPDATE commodity_master SET ";
		$Query .= " commodity							= '".$commodity."'";
		...
		$_SESSION[SUCCESS_MSG] = "Commodity updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
