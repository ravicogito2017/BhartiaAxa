...
function find_receipt_status($receipt_id)
{
	...
	//echo $invoice_id;
	$selTransaction = mysql_query("SELECT receipt_generated FROM direct_renewal_premium WHERE id='".$receipt_id."'");
				...
				<table>
					<tr>
					  ...
					  <td width="552" align="center">
						...
						<p style="font-size: 13px;" align="center">License No. - 5389734<?php 
	...
	<br/><br/>
	 ...
	 if(isset($getTransaction['applicant_name'])) {
		...
		<?php if(($getTransaction['sp_payment_mode'] == 'CHEQUE') || ($getTransaction['sp_payment_mode'] == 'DD')){?>Cheque/Demand Draft No <?php echo $getTransaction['sp_dd_no']; ?> dated <?php echo date('d-m-Y',strtotime($getTransaction['sp_dd_date'])); ?> drown on <?php echo $getTransaction['sp_dd_bank']; ?><?php }?>
			towards <?php if($getTransaction['is_duplicate']=='1'){echo "(Duplicate Crtificate";}else{echo "RP(Direct Renewal Premium";}?>) in relation to above policy for  & on behalf of Life Insurance Corporation of India under<strong> <?php echo $data['plan_name'];?> </strong> and term<strong> <?php echo $getTransaction['term']; ?></strong> Years.</div>
	<br/>
<br/><br/>
...
<?php
	mysql_query("UPDATE direct_renewal_premium SET receipt_generated=1 WHERE id='".base64_decode($_GET['id'])."'");
