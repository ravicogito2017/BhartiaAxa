...
function show_list(){
	...
	$sortField 		= loadVariable('sf','plan_name');
...
$where = " WHERE 1 ";
	...
	if($searchField=="plan_name"){
		$where .= " AND plan_name like '%".mysql_real_escape_string($searchString)."%' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from insurance_plan  ".$where;
...
$Query = "select  *  from insurance_plan ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
  ...
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="plan_name" <? if($searchField=='plan_name') echo 'selected';?>>Plan </option></td>
      <TD width="85%">
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['plan_name']?></TD>
	  <!-- <TD align=center>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM insurance_plan WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Plan <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="plan_name" id="plan_name" size="40" <? if(isset($rec[0]['plan_name']) && $rec[0]['plan_name']!="") {?> value="<?=stripslashes($rec[0]['plan_name'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>			<tr>				<td width="23%" align="right" valign="top" class="tbllogin">Is Latest Plan </td>				<td width="2%" align="center" valign="top" class="tbllogin">:</td>				<td width="75%" align="left" valign="top">					<!--<input type="text" class="validate[required] inplogin" name="plan_name" id="plan_name" size="40" <? if(isset($rec[0]['plan_name']) && $rec[0]['plan_name']!="") {?> value="<?=stripslashes($rec[0]['plan_name'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />--><input type="checkbox" name="is_new" id="is_new" value="1" <?php if(isset($rec[0]['is_new'])){ echo ($rec[0]['is_new'] == '1' ? 'checked ' : '');}?>>				</td>			  </tr>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from insurance_plan WHERE plan_name = '".$plan_name."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " INSERT INTO insurance_plan SET ";
	...
	else
	{
		...
		$Query  = "select * from insurance_plan WHERE plan_name = '".$plan_name."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			...
			if($is_new=="")			{			$is_new = 0;			}		
		$Query  = " UPDATE insurance_plan SET ";
		$Query .= " plan_name							= '".$plan_name."',";		$Query .= " is_new 								= '".$is_new."'";
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from insurance_plan WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE insurance_plan SET status = 0 WHERE id = ".$row_id."";
	...
	$Query  = "UPDATE insurance_plan SET status = 1 WHERE id = ".$row_id."";
	}
