...
if(isset($received) && count($received) > 0)
{
	...
	$receivedUpdate = mysql_query("UPDATE renewal_master_gtfs SET hub_received=1, hub_id=".$_SESSION[ADMIN_SESSION_VAR].", hub_receive_date='".$today."' WHERE id IN (".$receiveStr.") AND hub_id = 0");
}
...
if(isset($branch_despatched) && count($branch_despatched) > 0)
{
	...
	$sql= "UPDATE renewal_master_gtfs SET branch_despatched=1, branch_despatch_date='".$today."' WHERE id IN (".$branch_despatchedStr.")";
	#echo $sql;
	//exit;
...
/*if(isset($mode) && $mode == 'del') // delete transaction
{
	...
	mysql_query("UPDATE renewal_master_gtfs SET is_deleted=1 WHERE id=".$transaction_id);
	#echo "UPDATE renewal_master_gtfs SET is_deleted=1 WHERE id=".$transaction_id;
...
$_SESSION['branch_name_string'] = $complete_user_string;
	...
	if(isset($_SESSION['insured_name']) && $_SESSION['insured_name'] != '') 
	{
		...
		if($lastNameString != '')
		{
			$where.= ' AND insured_name like  IN %'.$_SESSION['insured_name'].'%';
		}
	}
...
//$selTransaction = mysql_query("SELECT id, branch_id, customer_id, deposit_date, comitted_amount, amount, tenure, transaction_charges, first_name, last_name, phone, email FROM renewal_master_gtfs WHERE is_deleted=0 ORDER BY id DESC");
...
?>
	<tr> 
			<td width="8%" align="center" valign="top" ><?php if(intval($rs[$i]['pis_id']) != 0){ ?><input type="checkbox" name="branch_despatched[]" value="<?php echo $mysql_id; ?>"<?php if($rs[$i]['branch_despatched'] == '1'){ echo 'disabled="disabled"';}?> <?php echo ($rs[$i]['branch_despatched'] == '1' ? 'checked' : '')?>
			...
			<td width="8%" align="center" valign="top" ><?php if($rs[$i]['health']=='5') echo "No"; 
			else
				echo "Yes";?></td>
