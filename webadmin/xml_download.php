...
$where = "WHERE is_deleted=0 AND migrated_from_dmspl != 2 AND payment_type != 'REDEMPTION' ";
...
if($numTransaction > 0)
{
	while($getTransaction = mysql_fetch_assoc($selTransaction))
	{
		...
		if($numFolio > 0)
		{
		 ...
		 $output .= "\t\t<HUSBANDNAME>".$getMasterRecord['husbands_name'];
		 $output .= "</HUSBANDNAME>\n";

		 $output .= "\t\t<MOTHERMAIDENNAME>".$getMasterRecord['mothers_maiden_name'];
		 $output .= "</MOTHERMAIDENNAME>\n";

		 $output .= "\t\t<ANNUALINCOME>".$getMasterRecord['annual_income'];
		 $output .= "</ANNUALINCOME>\n";

		 $output .= "\t\t<OCCUPATION>".$getMasterRecord['occupation'];
		 $output .= "</OCCUPATION>\n";

		 $output .= "\t\t<AGEPROOF>".$getMasterRecord['age_proof'];
		 $output .= "</AGEPROOF>\n";

		 $output .= "\t\t<ADDRESSPROOF>".$getMasterRecord['address_proof'];
		 $output .= "</ADDRESSPROOF>\n";

		 $output .= "\t\t<IDPROOF>".$getMasterRecord['id_proof'];
		 $output .= "</IDPROOF>\n";
		 ...
		 $output .= "\t\t<ADDRESSLINE1>".mysql_real_escape_string(xmlEscape($getMasterRecord['address1']));
		 $output .= "</ADDRESSLINE1>\n";
		 ...
		 $output .= "\t\t<NOMINEEADDRESS>".mysql_real_escape_string(xmlEscape($getFolioRecord['nominee_address']));
		 $output .= "</NOMINEEADDRESS>\n";

		 $output .= "\t\t<NOMINEEDOB>".$getFolioRecord['nominee_dob'];
		 $output .= "</NOMINEEDOB>\n";

		 $output .= "\t\t<APPOINTEENAME>".$getFolioRecord['appointee_name'];
		 $output .= "</APPOINTEENAME>\n";

		 $output .= "\t\t<APPOINTEERELATIONSHIP>".$getFolioRecord['appointee_relationship'];
		 $output .= "</APPOINTEERELATIONSHIP>\n";
		 ...
		 $output .= "\t\t<TRANSACTIONCHARGE>".$getTransaction['transaction_charges']; // Additional chage taken by the state, VAT etc.
		 $output .= "</TRANSACTIONCHARGE>\n";

		 //$output .= "\t\t<PENALTY>".$getTransaction['penalty'];
		 //$output .= "</PENALTY>\n";
