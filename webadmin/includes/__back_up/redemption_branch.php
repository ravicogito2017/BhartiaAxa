...
$Query = "SELECT id, branch_id, customer_id, deposit_date, amount,  transaction_charges, payment_mode, payment_type, application_no, receipt_number, folio_no_id, receipt_generated, pis_id, branch_despatched FROM installment_master ".$where.$OrderBY.$Limit;
...
<TABLE class="table_border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <tbody>
    ...
    <tr> 
      <td width="8%" align="center" valign="top" class="tbllogin">Despatch to Hub</td>
			...
			<td width="8%" align="center" valign="top" class="tbllogin">View Scanned Image</td>
    </tr>
<?php
...
?>
	<tr> 
      <td width="8%" align="center" valign="top" ><?php if(intval($rs[$i]['pis_id']) != 0){ ?><input type="checkbox" name="branch_despatched[]" value="<?php echo $mysql_id; ?>" <?php echo ($rs[$i]['branch_despatched'] == '1' ? 'checked' : '')?>> <?php } ?></td>
			...
			<?php if(strtoupper($rs[$i]['payment_type']) != 'SUBSEQUENT PAYMENT') { ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_edit.php?id=<?php echo base64_encode($mysql_id); ?>')">Edit<?php } ?></td> -->
			...
			<td width="8%" align="center" valign="top" ><a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_upload.php?id=<?php echo base64_encode($rs[$i]['folio_no_id']); ?>')">View</td>
    </tr>
