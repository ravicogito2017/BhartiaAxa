...
$where = " WHERE commodity_id ='".SILVER_ID."' ";
...
$Query = "SELECT t.id as id, t.tax_percentage AS tax_percentage, p.place as place_id FROM tax_master AS t, place_master AS p WHERE t.commodity_id='".SILVER_ID."' AND t.place_id = p.id ORDER BY p.place ASC".$Limit;
#echo $Query ;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Silver Tax Management Panel</TD>
    </TR>
    <TR> 
	  ...
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=silver_tax&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=silver_tax&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
    ...
    <tbody>
      ...
      <tr class="text4"> 
		...
		<?
			if($user_id == "")
			{
				echo "Add Tax Percentage for Silver";
		    }
			else
			{
				echo "Edit Tax Percentage for Silver";
			}		
		?>
		 </td>
      </tr>
...
function  update_record($row_id = '')
{
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from tax_master WHERE place_id = '".$place_id."' AND commodity_id='".SILVER_ID."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
	...
	else
	{
		...
		$Query  = "select * from tax_master WHERE place_id = '".$place_id."' AND commodity_id='".SILVER_ID."' and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
