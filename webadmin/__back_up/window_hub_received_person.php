...
if(isset($_GET['id']) && !empty($_GET['id']))
{
	...
	if(isset($_POST['hub_receive_person_name']) && ($_POST['hub_receive_person_name'] != '') && isset($_POST['hub_receive_person_emp_code']) && ($_POST['hub_receive_person_emp_code'] != ''))
	{
		#echo '<pre>';
		#print_r($_POST);
		#echo '</pre>';
		...
		extract($_POST);
				...
				$folio_query = "UPDATE customer_folio_no SET hub_receive_person_name = '".$hub_receive_person_name."', hub_receive_person_emp_code = '".$hub_receive_person_emp_code."', hub_material_receive_date = '".date('Y-m-d')."' WHERE id='".$folio_id."'";
...
<?php
	...
	$getSumData = mysql_fetch_array($selSumData);
			...
			$hub_receive_person_name = $getFolioRecord['hub_receive_person_name']; 
			$hub_receive_person_emp_code = $getFolioRecord['hub_receive_person_emp_code']; 
...
<html>
 <head>
  <title> View Receiving Person for Hub/Division </title>
...
<!--
	...
	{	
		...
		if(document.addForm.hub_receive_person_name.value.search(/\S/) == -1)
		{
			alert("Please Enter Hub Receiver Name");
			document.addForm.hub_receive_person_name.focus();
			return false;
		}	
		if(document.addForm.hub_receive_person_emp_code.value.search(/\S/) == -1)
		{
			alert("Please Enter Hub Receiver EMP Code");
			document.addForm.hub_receive_person_emp_code.focus();
			return false;
		}	
	}
//-->
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">View Receiving Person for Hub/Division</td>
    </tr>
    ...
    <tr> 
      ...
      <td valign="top" align="left"><input type="text" class="inplogin" name="delivery_person_emp_code" <?php echo $otherReadonly; ?> value="<?php echo $delivery_person_emp_code; ?>" onblur="this.value = this.value.toUpperCase();"></td>
		...
		<tr> 
      <td class="tbllogin" valign="top" align="right">Receiver Name for HUB<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="text" class="inplogin" name="hub_receive_person_name" value="<?php echo $hub_receive_person_name; ?>" onblur="this.value = this.value.toUpperCase();"></td>
    </tr>
		<tr> 
      <td class="tbllogin" valign="top" align="right">Receiver Emp Code for HUB<font color="#ff0000">*</font></td>
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input type="text" class="inplogin" name="hub_receive_person_emp_code" value="<?php echo $hub_receive_person_emp_code; ?>" onblur="this.value = this.value.toUpperCase();"></td>
    </tr>
    ...
    <?php if(($hub_receive_person_name == '') && ($hub_receive_person_emp_code == '')) { ?>
		<tr> 
      <td colspan="2">&nbsp;</td>
