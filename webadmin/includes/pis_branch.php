...
if(isset($_GET['status']) && $_GET['status'] == 'del' && $_GET['did'] != "") {
    ...
    $update = "update pis_master set is_deleted = '1' where id = ".$id;    
    $objDB->setQuery($update);
    $objDB->update();        
    
    $select = "select pis_mode from pis_master where id = ".$id;    
    ...
    $update1 = "update installment_master_branch set  ".$updcol."  = '0' where  $updcol = ".$id;     
    $objDB->setQuery($update1);
    $objDB->update(); 
    
    header("Location:".URL."webadmin/index.php?p=pis_branch");   
				...
				<tr>
					...
					<tr>
						...
						{ //echo $pageRecordCount;
							if($_SESSION[ROLE_ID] != '7' && $_SESSION[ROLE_ID] != '3' && $_SESSION[ROLE_ID] != '1'){  //============ Edited  =============
						?>
...
?>
	...
	<tr> 			
			...
			<a href="<?php echo URL; ?>webadmin/index.php?p=pis_branch&status=del&did=<?php echo base64_encode($mysql_id); ?>" onclick="return confirm('Do you want to delete this item?');">Cancel</a>
			</td>
			<?php } ?>
    </tr>
<?php		
