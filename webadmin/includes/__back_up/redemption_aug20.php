...
if(isset($received) && count($received) > 0)
{
	...
	$receivedUpdate = mysql_query("UPDATE customer_folio_no SET hub_received=1, hub_id=".$_SESSION[ADMIN_SESSION_VAR].", hub_receive_date='".$today."' WHERE id IN (".$receiveStr.") AND hub_id = 0");
}
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=find_customer_id_through_id($rs[$i]['customer_id'])?></TD>
		...
		if(intval($_SESSION[ROLE_ID]) != 4) // NOT FOR BRANCH
			{ 
	?>
			<td width="8%" align="center" valign="top" ><input type="checkbox" name="received[]" value="<?php echo $rs[$i]['id']; ?>" <?php echo ($rs[$i]['hub_received'] == '1' ? 'checked' : '')?>></td>
			...
			<td width="10%" align="center" valign="top" ><a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL;?>webadmin/window_hub_delivery_person.php?id=<?php echo base64_encode($rs[$i]['id']);?>')">Add</td>
